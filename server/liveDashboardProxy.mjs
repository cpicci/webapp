...
function buildSnapshot(casesRows, relanceRows, comptaRows) {
  return {
    meta: {
    ...
    cases: casesRows.map((row) => ({
      ...
      motif: row['Motif'],
      statut: row['Statut'],
      priorite: row['PrioritÃ©'],
      resume: row['RÃ©sumÃ©'],
      nomCopro: row['Nom Copro'],
      lot: row['NÂ° Lot'],
      contactEmail: row['Contact (email)'],
      createdAt: row['CrÃ©e le'],
...
export async function getLiveDashboardSnapshot() {
  ...
  const payload = buildSnapshot(
    rowsToObjects(casesRaw),
    rowsToObjects(relanceRaw),
    rowsToObjects(comptaRaw),
  );
